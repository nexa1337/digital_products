...
// Visitor Counter
let visitorCount = Number.parseInt(localStorage.getItem("visitors") || "132")
...
function startVisitorUpdates() {
  // Update visitor count every 3-7 seconds
  visitorUpdateInterval = setInterval(
    () => {
      const oldCount = visitorCount

      // Generate realistic delta (-12 to +12)
      let delta = Math.floor(Math.random() * 25) - 12

      // Bias towards increases during daytime (optional)
      const hour = new Date().getHours()
      if (hour >= 9 && hour <= 17) {
        if (Math.random() > 0.4) delta = Math.abs(delta)
      }

      visitorCount = Math.max(1, visitorCount + delta)

      // Animate the number change with smooth counter tween
...
function renderProducts(category) {
  ...
  const startIdx = (currentPage[category] - 1) * itemsPerPage
        ...
        <div class="product-card">
            ...
            <div class="product-info">
                ...
                <div class="product-footer">
                    ...
                    <button class="quick-view-btn" onclick="openQuickView(${product.id}, '${category}')">Quick View</button>
                </div>
            </div>
        </div>
    `,
